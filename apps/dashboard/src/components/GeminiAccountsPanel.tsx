...
export function GeminiAccountsPanel() {
  const list = trpc.geminiAccounts.list.useQuery(undefined, {
    refetchInterval: 15_000,
  });
  const del = trpc.geminiAccounts.delete.useMutation({
    onSuccess: () => list.refetch(),
  });
  const rename = trpc.geminiAccounts.rename.useMutation({
    onSuccess: () => list.refetch(),
  });
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="font-semibold">Gemini accounts</h2>
          <p className="text-xs text-(--color-text-muted)">
            Google OAuth tokens — proxy rotates across these per request. Free
            tier: gemini-2.5-flash, gemini-3-flash-preview, etc.
          </p>
        </div>
                ...
                <tr key={a.id} className="border-b border-(--color-border)/50">
                  <td className="py-2 pr-3">
                    {editingId === a.id ? (
                      ...
                      >
                        ...
                        <div className="text-[11px] text-(--color-text-muted) font-mono hidden md:block">
                          {a.id}
                        </div>
                      </button>
                    )}
                  </td>
                  <td className="py-2 pr-3 text-(--color-text-muted) text-xs">
                    {a.email ?? "—"}
                  </td>
                  <td
                  ...
                  <td className="py-2 pr-3 text-right">
                    <Button
                      variant="danger"
                      onClick={() => {
                        if (confirm(`Delete account "${a.label || a.email || a.id}"?`)) {
                          del.mutate({ id: a.id });
                        }
                      }}
      ...
      {list.data && list.data.accounts.length === 0 && (
        <Card>
          <div className="text-sm text-(--color-text-muted) text-center py-4">
            No Gemini accounts yet — click <em>Add account</em> to start the
            Google OAuth flow.
          </div>
        </Card>
      )}

      <Modal open={showAdd} onClose={() => setShowAdd(false)} title="Add Gemini account">
        <AddAccountFlow
          onDone={async () => {
            await list.refetch();
            setShowAdd(false);
...
function AddAccountFlow({ onDone }: { onDone: () => Promise<void> | void }) {
  ...
  const start = trpc.geminiAccounts.startOAuth.useMutation();
  const poll = trpc.geminiAccounts.pollOAuth.useMutation();
  const cancel = trpc.geminiAccounts.cancelOAuth.useMutation();
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      ...
      if (Date.now() > session.expiresAt) {
        ...
        setErr("Session expired. Try again.");
        return;
      }
      try {
        ...
        if (res.status === "failed") {
          setErr(`Login failed: ${("error" in res && res.error) || "unknown"}`);
          return;
        }
    ...
    return (
      <div className="space-y-4">
        <div className="bg-(--color-bg) border border-(--color-border) rounded px-4 py-3 text-sm">
          <p className="mb-3">
            A new tab should have opened. If not, click the link below or copy
            it:
          </p>
          ...
          <div className="text-xs text-(--color-text-muted)">
            ...
            <code className="font-mono">{session.redirectUri}</code> and the
            admin server captures the code automatically.
          </div>
        </div>
        ...
        <div className="text-sm">
          ...
          {pollStatus === "expired" && (
            <div className="text-(--color-danger)">Session expired.</div>
          )}
          {pollStatus === "failed" && (
            <div className="text-(--color-danger)">Login failed.</div>
          )}
        </div>
  ...
  return (
    <form
      ...
      onSubmit={async (e) => {
        ...
        try {
          ...
          const callbackHost = new URL(res.redirectUri).hostname;
          if (isLoopbackHost(callbackHost) && isRemoteDashboard) {
            await cancel.mutateAsync({ sessionId: res.sessionId });
            setErr(
              "Gemini callback is configured for 127.0.0.1, but this dashboard is open from another device. On Android, 127.0.0.1 points to the phone, not the admin server. Open the dashboard on the server machine, or set GEMINI_OAUTH_CALLBACK_BASE to a reachable admin URL with a matching Google OAuth redirect URI.",
            );
            return;
          }
          setSession({
    ...
    >
      ...
      <div className="text-xs text-(--color-text-muted) bg-(--color-bg-elev) border border-(--color-border) rounded px-3 py-2">
        <strong>Note:</strong> The default Gemini OAuth callback uses{" "}
        <code className="font-mono">127.0.0.1</code>, so login must be opened on
        the same machine as the admin server. For Android or another device, set{" "}
        <code className="font-mono">GEMINI_OAUTH_CALLBACK_BASE</code> to a
        reachable admin URL and configure the same redirect URI in Google OAuth.
      </div>
      {err && <div className="text-(--color-danger) text-sm">{err}</div>}
      <div className="flex justify-end gap-2 pt-2">
        <Button type="submit" disabled={start.isPending}>
          {start.isPending ? "Starting…" : "Start Google login"}
        </Button>
      </div>
    </form>
  );
